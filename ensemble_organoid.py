...
Output is written to results_organoid/
"""
...
def main():
    ...
    proba_paths = {
        'rf':  rf"{BASE}\results_organoid\rf_proba.npy",
        'xgb': rf"{BASE}\results_organoid\xgb_proba.npy",
    ...
    # Save to results_organoid
    os.makedirs(rf"{BASE}\results_organoid", exist_ok=True)
    np.save(rf"{BASE}\results_organoid\ensemble_proba.npy", ensemble_proba)
    json.dump(metrics, open(rf"{BASE}\results_organoid\ensemble_test.json", 'w'), default=float)
    print(f"\n✓ Saved results_organoid/ensemble_test.json and results_organoid/ensemble_proba.npy")
